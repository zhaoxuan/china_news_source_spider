...
origin_url = 'http://news.baidu.com/ns?cl=2&rn=50&tn=news&word=%s'
...
class BaiduNewsSpider(scrapy.Spider):
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        ...
        spider = super(BaiduNewsSpider, cls).from_crawler(crawler, *args, **kwargs)
        crawler.signals.connect(spider.closed_handler, signals.spider_closed)

        return spider

    def closed_handler(self, spider):
        spider.logger.warning('BaiduNewsSpider closed, success: %d, failure: %d' % (self.success, self.failure))

    def parse(self, response):
        lists = response.xpath('//div[@class="result"]')

        for item in lists:
            url = item.xpath('h3/a/@href').extract()[0]
            name = item.xpath('.//p[@class="c-author"]/text()').extract()[0]
