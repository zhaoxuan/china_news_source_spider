...
origin_url = 'http://news.chinaso.com/newssearch.htm?q=%s'
dir_path = os.path.dirname(os.path.realpath(__file__))
dicts_path = dir_path + '/../../../dicts.txt'


class ChinasoNewsSpider(scrapy.Spider):
    name = 'chinaso_news_spider'
    allowed_domains = ['chinaso.com']
    start_urls = []
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        ...
        spider = super(ChinasoNewsSpider, cls).from_crawler(crawler, *args, **kwargs)
        crawler.signals.connect(spider.closed_handler, signals.spider_closed)

        return spider

    def closed_handler(self, spider):
        spider.logger.warning('ChinasoNewsSpider closed, success: %d, failure: %d' % (self.success, self.failure))

    def parse(self, response):
        lists = response.xpath('//ol[@class="seResult"]/li')

        for item in lists:
            url = item.xpath('.//h2/a/@href').extract()[0]
            name = item.xpath('.//p[@class="snapshot"]/span/text()').extract()[0]
            domain = urlparse(url).netloc
            spider_item = SpiderItem()
            spider_item['domain'] = domain
            spider_item['name'] = name.split()[-1]
